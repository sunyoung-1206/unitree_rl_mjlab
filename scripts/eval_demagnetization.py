"""Demagnetization fault injection evaluation.

Usage:
  conda activate mjlab
  python scripts/eval_demagnetization.py [--num-envs 64] [--num-steps 1000] [--device cpu]
"""
...
OUTPUT_DIR = Path("results/demagnetization_aplus")
...
def run_eval(task_name: str, demag: float, args) -> dict:
    ...
    if task_name != "PD":
        mean_abs_current = float(np.mean(np.abs(stacked["act"])))
    else:
        mean_abs_current = 0.0
    mean_abs_torque = float(np.mean(np.abs(stacked["qfrc_actuator"][:, 6:])))  # skip floating base 6 DOFs
    ...
    summary = {
        ...
        "mean_abs_current": mean_abs_current,
    ...
    for task_name in args.tasks:
        for demag in args.demag_factors:
            ...
            print(f"  Return={summary['episode_return']:.1f}  "
                  ...
                  f"|I|={summary['mean_abs_current']:.3f}  "
                  f"|τ|={summary['mean_abs_torque']:.3f}  "
