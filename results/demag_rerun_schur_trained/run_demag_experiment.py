...
def run_one(args) -> None:
    device = args.device
    info = dict(POLICIES[args.policy])
    if args.policy == "methoda" and args.methoda_checkpoint:
        info["checkpoint"] = args.methoda_checkpoint
    if args.policy == "pd" and args.pd_checkpoint:
        info["checkpoint"] = args.pd_checkpoint
    print(f"[INFO] checkpoint = {info['checkpoint']}")
...
def main():
    ...
    parser.add_argument("--methoda-checkpoint", default=None,
                        help="override the methoda policy checkpoint path "
                             "(default: hardcoded POLICIES['methoda']['checkpoint']).")
    parser.add_argument("--pd-checkpoint", default=None,
                        help="override the pd policy checkpoint path.")
