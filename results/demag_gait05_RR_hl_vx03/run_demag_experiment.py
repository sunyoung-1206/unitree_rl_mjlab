...
POLICIES = {
    ...
    "aplus": {
        ...
        "checkpoint": "logs/rsl_rl/aplus_dynprm4/"
                      "2026-04-28_13-27-02_seed42/model_1999.pt",
        "policy_dt": 0.020,
    },
    # A+ + driver-rate torque-tracking integral loop. 관측/액션 공간이
    # Coupled-Electric 와 동일하므로 같은 PPO 체크포인트 재사용.
    # 정책은 적분기 존재를 모르고 학습됐지만 추론 단계에서 적분기가 동작.
    "aplus_tloop": {
    ...
    # 2026-07-16 학습: DeployDR-Gait05 curriculum/reward 위에 액추에이터만
    # Method A 전기모터로 교체한 태스크 (foot_gait weight 0.50). 최종 체크포인트
    # model_4999.pt. decimation=200 · physics dt=0.1ms → policy_dt=20ms.
    "gait05": {
        "task_id": "Unitree-Go2-Flat-MethodA-Electric-DeployDR-Gait05-v0",
        "checkpoint": "logs/rsl_rl/go2_methoda_deploydr_gait05/"
                      "2026-07-16_13-05-31_seed42/model_4999.pt",
        "policy_dt": 0.020,
    },
}

# Electric-actuator policies (NativeElectricActuator-based).
# Used to gate all branches that depend on _Ktgr / _tau_des_hold / etc.
ELECTRIC_POLICIES = {"methoda", "aplus", "aplus_tloop", "gait05"}

# Kt = Ke = 0.26 N*m/A (motor-side), gear_ratio = 6.33 -- see
# src/assets/robots/unitree_go2/go2_constants.py:_GO2_MOTOR_PHYS, used by
# GO2_METHODA_CALF (verified live: env.scene["robot"].actuators[calf]._Ktgr
# == 1.6458 before any demag injection). The previous 0.128 * 6.33 = 0.8102
# constant here was stale (off by 0.26/0.128 = 2.03x) and silently changed
# both the injected demag severity and every downstream tau/I derived
# quantity -- see results/demag_gait05_RR_hl_vx03/view_data.ipynb for the
# corrected re-run and impact analysis.
KT_NOMINAL_JOINT = 0.26 * 6.33
KE_NOMINAL_JOINT = 0.26 * 6.33
...
def apply_fixed_velocity(env, device, vx=0.5, vy=0.0,
    ...
    학습 분포의 25 % heading-controlled 슬라이스와 동일한 상태로 추론하기 위해
    `is_heading_env=True`, `heading_target` 을 매 step 갱신. 공통 헬퍼는
    `src.utils.heading_lock.apply_heading_lock_velocity` 에 있음.

    Args:
        target_heading: None → 첫 step 의 base yaw 자동 캡처 (auto mode).
                        float → 명시적 target [rad] (explicit mode).
        no_heading_control: True → 기존 동작(wz=0 고정, is_heading_env=False)
                            으로 fallback (비교 실험용).
    """
...
def run_one(args) -> None:
    ...
    if args.schur_test and args.policy in ELECTRIC_POLICIES:
        changed = 0
    ...
    if args.policy in ELECTRIC_POLICIES and args.vbus_override is not None:
        n_vb = 0
        for act_cfg in env_cfg.scene.entities["robot"].articulation.actuators:
            if hasattr(act_cfg, "V_bus"):
                act_cfg.V_bus = float(args.vbus_override)
                n_vb += 1
        print(f"[VBUS OVERRIDE] V_bus -> {args.vbus_override} V on {n_vb} actuator cfgs "
              f"(counterfactual: removes/relaxes voltage clamp to isolate its effect).")
    ...
    if args.policy in ELECTRIC_POLICIES:
        if args.leg == "none":
            print(f"[INFO] {args.policy} healthy — no demag applied.")
        else:
            inject_demagnetization(env, args.leg, args.demag_factor)
            print(f"[INFO] {args.policy} {args.leg}_calf plant Kt·gr / Ke·gr × "
    ...
    # apply_fixed_velocity 는 wrapped.reset() 후로 미룸 (reset_event 가 base 를
    # 무작위로 덮어쓴 다음 force_deterministic_initial_state 가 결정론적 값으로
    # 다시 덮어쓴 뒤에 _update_command 패치를 걸어야 stored_target=0.0 이 안전).
    ...
    vbus_suffix = (f"_vbus{int(round(args.vbus_override)):03d}"
                   if args.vbus_override is not None else "")
    output_policy_tag = (args.policy + ki_suffix + vbus_suffix
    ...
    # 모든 fault 케이스가 동일 (pos=(0,0,h), quat=(1,0,0,0), vel=0, joint=default)
    # 에서 출발. 공통 헬퍼: src.utils.init_state.apply_zero_initial_state.
    from src.utils.init_state import apply_zero_initial_state
    apply_zero_initial_state(env, verbose=True)

    obs, _ = wrapped.reset()
    # 'nan' 입력 → auto 모드 (None) 로 변환.
    ...
    meta = {
        ...
        "vbus_override": (float(args.vbus_override) if args.vbus_override is not None else None),
    ...
    # ── Validation ratio (electric policy demag only) ──────────────
    if args.policy in ELECTRIC_POLICIES and args.leg != "none" and args.demag_factor < 1.0:
...
def main():
    ...
    parser.add_argument("--wz", type=float, default=0.0,
                        help="DEPRECATED: heading-lock 활성 시 무시됨. "
                             "비활성하려면 --no-heading-control 사용.")
    parser.add_argument("--target-heading", type=float, default=0.0,
                        help="heading-lock target [rad]. 기본 0.0 (force-init "
                             "이 quat=(1,0,0,0) 으로 yaw=0 강제하므로 일치). "
                             "None 의도면 'nan' 입력 → auto 모드.")
    parser.add_argument("--no-heading-control", action="store_true",
                        help="구 동작(wz=0 고정, is_heading_env=False)으로 "
                             "fallback. 비교 실험용.")
    ...
    parser.add_argument("--vbus-override", type=float, default=None,
                        help="override actuator V_bus [V] (electric policies only). "
                             "Counterfactual: e.g. a very large value removes the "
                             "voltage clamp to isolate its effect on gait. Output "
                             "dir gets _vbus<int> suffix.")
